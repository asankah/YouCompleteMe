...
class CommandRequest(BaseRequest):
  ...
  def __init__(self, arguments, ycmd_proxy, completer_target=None):
    super(CommandRequest, self).__init__(ycmd_proxy)
    self._arguments = _EnsureBackwardsCompatibility(arguments)
    self._completer_target = (completer_target if completer_target else
                              'filetype_default')
    self._response = None
  ...
  def Start(self):
    ...
    with HandleServerException(self._ycmd):
      self._response = self._ycmd.RunCompleterCommand(request_data).result()
  ...
  def _HandleFixitResponse(self):
    if not len(self._response['fixits']):
      ...
      try:
        ...
        vimsupport.ReplaceChunks(self._response['fixits'][fixit_index][
            'chunks'])
...
def SendCommandRequest(arguments, completer, ycmd_proxy):
  request = CommandRequest(arguments, ycmd_proxy, completer)

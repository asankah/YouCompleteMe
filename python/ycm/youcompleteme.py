...
from ycmd.embedded_server import StartEmbedded
...
class YouCompleteMe(object):
  ...
  def _SetupLocalServer(self):
    self._available_completers = {}
    self._user_notified_about_crash = False
    self._ycmd = StartEmbedded(self._user_options)
  ...
  def _SetupLogging(self):
    ...
    def FreeFileFromOtherProcesses(file_object):
      if utils.OnWindows():
        ...
        windll.kernel32.SetHandleInformation(file_handle, HANDLE_FLAG_INHERIT,
                                             0)
    ...
    if (not self.NativeFiletypeCompletionAvailable() and
      ...
      if self._omnicomp.ShouldUseNow(wrapped_request_data):
        self._latest_completion_request = OmniCompletionRequest(
            self._omnicomp, wrapped_request_data, self._ycmd)
  ...
  def SendCommandRequest(self, arguments, completer):
    if self.IsServerAlive():
      return SendCommandRequest(arguments, completer, self._ycmd)

  def GetDefinedSubcommands(self):
    if self.IsServerAlive():
      with HandleServerException(self._ycmd):
        return self._ycmd.GetDefinedSubcommands().result()
    return []
  ...
  def FiletypeCompleterExistsForFiletype(self, filetype):
    ...
    exists_completer = SendCompleterAvailableRequest(filetype, self._ycmd)
  ...
  def OnFileReadyToParse(self):
    ...
    self._latest_file_parse_request = EventNotification(
        'FileReadyToParse', self._ycmd, extra_data=extra_data)
  ...
  def OnBufferUnload(self, deleted_buffer_file):
    ...
    SendEventNotificationAsync(
        'BufferUnload', self._ycmd, filepath=deleted_buffer_file)
  ...
  def OnBufferVisit(self):
    ...
    SendEventNotificationAsync('BufferVisit', self._ycmd, extra_data=extra_data)

  def OnInsertLeave(self):
    if not self.IsServerAlive():
      return
    SendEventNotificationAsync('InsertLeave', self._ycmd)
  ...
  def OnCurrentIdentifierFinished(self):
    if not self.IsServerAlive():
      return
    SendEventNotificationAsync('CurrentIdentifierFinished', self._ycmd)
  ...
  def _FilterToMatchingCompletions_NewerVim(self, completions, full_match_only):
    ...
    for completion in completions:
      ...
      match_keys = (['word', 'abbr', 'menu', 'info'] if full_match_only else
                    ['word'])
  ...
  def _FilterToMatchingCompletions_OlderVim(self, completions, full_match_only):
  ...
  def _HasCompletionsThatCouldBeCompletedWithMoreText_NewerVim(self,
    ...
    for completion in completions:
      word = utils.ToUnicode(ConvertCompletionDataToVimData(completion)['word'])
  ...
  def _OnCompleteDone_Csharp(self):
    ...
    if len(namespaces) > 1:
      choices = ['{0} {1}'.format(i + 1, n) for i, n in enumerate(namespaces)]
  ...
  def _GetRequiredNamespaceImport(self, completion):
    if ('extra_data' not in completion or
        'required_namespace_import' not in completion['extra_data']):
      return None
  ...
  def DiagnosticUiSupportedForCurrentFiletype(self):
    return any(
        [x in DIAGNOSTIC_UI_FILETYPES for x in vimsupport.CurrentFiletypes()])
  ...
  def FileParseRequestReady(self, block=False):
    return bool(
        self._latest_file_parse_request and
  ...
  def ShowDetailedDiagnostic(self):
    if not self.IsServerAlive():
      return
    with HandleServerException(self._ycmd):
      detailed_diagnostic = self._ycmd.GetDetailedDiagnostic(BuildRequestData(
      )).result()

      if 'message' in detailed_diagnostic:
        vimsupport.PostVimMessage(detailed_diagnostic['message'], warning=False)

  def DebugInfo(self):
    debug_info = ''
    if self._client_logfile:
      debug_info += 'Client logfile: {0}\n'.format(self._client_logfile)
    debug_info += self._ycmd.DebugInfo(BuildRequestData()).result()
  ...
  def _AddExtraConfDataIfNeeded(self, extra_data):
    ...
    if extra_conf_vim_data:
      extra_data['extra_conf_data'] = BuildExtraConfData(extra_conf_vim_data)
